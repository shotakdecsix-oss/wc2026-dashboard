...
"""
    ...
    prompts = {
        'groupPrediction1': (
            context
            + "\n## タスク\n"
            "グループA〜Fについて以下を予想してください。\n\n"
            "【重要ルール】\n"
            "- 各グループの1位・2位は自動でラウンド16進出\n"
            "- 各グループの4位は全員敗退（グループ内最下位）\n"
            "- 各グループの3位（計12チーム）のうち成績上位8チームがラウンド16進出、下位4チームが敗退\n\n"
            "各グループ（A〜F）について以下の形式で回答:\n"
            "1位：[チーム名]（理由）\n"
            "2位：[チーム名]（理由）\n"
            "3位：[チーム名]（3位通過または敗退の見込み）\n"
            "4位：[チーム名]（敗退確定・グループ最下位の理由）\n\n"
        ...
        'groupPrediction2': (
            context
            + "\n## タスク\n"
            "グループG〜Lについて以下を予想してください。\n\n"
            "【重要ルール】\n"
            "- 各グループの1位・2位は自動でラウンド16進出\n"
            "- 各グループの4位は全員敗退（グループ内最下位）\n"
            "- 各グループの3位（計12チーム）のうち成績上位8チームがラウンド16進出、下位4チームが敗退\n\n"
            "各グループ（G〜L）について以下の形式で回答:\n"
            "1位：[チーム名]（理由）\n"
            "2位：[チーム名]（理由）\n"
            "3位：[チーム名]（3位通過または敗退の見込み）\n"
            "4位：[チーム名]（敗退確定・グループ最下位の理由）\n\n"
            "最後に以下のセクションを必ず含めること:\n"
            "## 全体3位中の敗退予想\n"
            "【厳守】このセクションには、上記でグループG〜Lの3位と予想したチームのみを記載すること。"
            "4位と予想したチームをここに含めることは絶対に禁止。\n"
            "グループA〜Fの3位予想チームもここで考慮し、12グループすべての3位チームから最も敗退可能性が高い4チームを選ぶ。\n"
            "敗退予想4チーム：[チーム名×4]\n"
            "各チームの理由：3位同士の比較で勝ち点・得失点差・総得点が不利な根拠を明記。\n\n"
            "現在の大会結果データを最大限活用し、まだ終了していないグループは現時点の試合結果から傾向を分析して予想してください。"
            "回答は600字以内で日本語で。箇条書き（- ）を使い、**キーワード**は太字で。"
        ),
        'japanScenario': (
            context
            + "\n## タスク\n"
