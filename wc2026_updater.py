...
"""
    ...
    prompts = {
        'groupPrediction1': (
            ...
            + "\n## タスク\n"
            "グループA〜Fについて予想してください。\n\n"
            "【出力ルール】各グループは必ず以下の4行のみ。理由・説明は不要（STEP2でのみ記述）：\n\n"
            "グループX:\n"
            "- 1位: [チーム名]（自動通過）\n"
            "- 2位: [チーム名]（自動通過）\n"
            "- 3位: [チーム名]（3位通過争い参加）\n"
            "- 4位: [チーム名]（グループ最下位、無条件敗退）\n\n"
            "グループA〜Fの6グループ分を上記フォーマットで出力してください。\n\n"
            "---\n"
            "## STEP 1: 自分が予想した3位チームを確認（必ず記入）\n"
            "- A-3位: [グループAで3位と予想したチーム]\n"
            "- B-3位: [グループBで3位と予想したチーム]\n"
            "- C-3位: [グループCで3位と予想したチーム]\n"
            "- D-3位: [グループDで3位と予想したチーム]\n"
            "- E-3位: [グループEで3位と予想したチーム]\n"
            "- F-3位: [グループFで3位と予想したチーム]\n\n"
            "## STEP 2: 3位チーム敗退予想（STEP 1の6チームのみから2チームを選ぶ）\n"
            "敗退予想（3位チームから2チーム）：\n"
            "- **[STEP 1に記載したチーム名]**（理由）\n"
            "- **[STEP 1に記載したチーム名]**（理由）\n\n"
            "【絶対厳守】STEP 2の敗退予想には、STEP 1に記載したチーム名のみを使うこと。4位チームを含めることは絶対禁止。\n\n"
        ...
        'groupPrediction2': (
            ...
            "- 3位: [チーム名]（3位通過争い参加）\n"
            "- 4位: [チーム名]（グループ最下位、無条件敗退）\n\n"
            "グループG〜Lの6グループ分を上記フォーマットで出力してください。\n\n"
            "---\n"
            "## STEP 1: 自分が予想した3位チームを確認（必ず記入）\n"
            "- G-3位: [グループGで3位と予想したチーム]\n"
            "- H-3位: [グループHで3位と予想したチーム]\n"
            "- I-3位: [グループIで3位と予想したチーム]\n"
            "- J-3位: [グループJで3位と予想したチーム]\n"
            "- K-3位: [グループKで3位と予想したチーム]\n"
            "- L-3位: [グループLで3位と予想したチーム]\n\n"
            "## STEP 2: 3位チーム敗退予想（STEP 1の6チームのみから4チームを選ぶ）\n"
            "敗退予想（3位チームから4チーム）：\n"
            "- **[STEP 1に記載したチーム名]**（理由）\n"
            "- **[STEP 1に記載したチーム名]**（理由）\n"
            "- **[STEP 1に記載したチーム名]**（理由）\n"
    ...
    LARGE_TOKEN_KEYS = {'groupPrediction1', 'groupPrediction2'}
    result = {}
    for key, prompt in prompts.items():
        try:
            resp = client.messages.create(
                model='claude-haiku-4-5-20251001',
                max_tokens=1800 if key in LARGE_TOKEN_KEYS else 800,
